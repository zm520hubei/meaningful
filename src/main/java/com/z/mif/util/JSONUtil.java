...
public class JSONUtil {
	...
	static {
        ...
        mapper.getSerializationConfig().setDateFormat(DateFormat.getDateFormat(DateFormat.DATETIME_ISO_FORMAT));
	}
	 ...
	 * @throws ProcessException
	 */
	public static <T> List<T> encodeList(String json,Class<T> clz) throws Exception{
		try {
			return mapper.readValue(json,new TypeReference<List<T>>() {});
		} catch (IOException e) {
			throw new Exception("JSON convert to List failed.", e);
		}
	}
	
	public static String uncode(Object obj,String prexfie) throws Exception{
		return uncode(obj).replaceAll("\\{\\\"", "\\{\""+prexfie).replaceAll(",\\\"", ",\""+prexfie);
	}
