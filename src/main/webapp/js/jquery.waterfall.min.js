...
(function($) {
    var $window = $(window),
        ...
        defaults = {
            ...
            spacingWidth: 20, // the brick element horizontal spacing
    ...
    Waterfall.prototype = {
        ...
        _init: function () {
            ...
            $window.onload =  $this._positionAll();
        ...
        _getColumnCount: function () {
            var parentWidth = this.$element.width(),//获得容器宽度
                $item = $(this.options.itemClass),//获得需要定位的class
                itemWidth = $item.eq(0).outerWidth(), //获得定位的第一个的宽度包括内边框  参数为true则 也包括外边框margin
                iCol = Math.floor(parentWidth / (itemWidth + this.options.spacingWidth)),//一行显示多少列
                realWidth = 0,
                leftOffset = 0;
        ...
        _positionAll: function () {
            ...
            $item.each(function(index) {
                ...
                if(index < $this.cols) {
					...
					var wl = $window.width()*0.05;
                    $(this).css("top", minHeight + $this.options.spacingHeight);
                    $(this).css("left", $item.eq(minIndex).offset().left - wl);
